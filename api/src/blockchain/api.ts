...
export const createTransactionOnBlockchain = async (fileBuffer: Buffer): Promise<{ fileHash: string, block: number }> => {
  ...
  }).then(res => res.json() as unknown as { fileHash: string, block: number }).catch(err => {
    console.error(err);
...
export const verifyDocumentOnBlockchain = async (fileHash: string, block: number): Promise<boolean> => {
  ...
  return fetch(`${process.env.BLOCKCHAIN_ORIGIN_NODE}/transaction/verify/${block}/${fileHash}`).then(res => res.json() as unknown as { valid: boolean }).then(data => data.valid).catch(err => {
    console.error(err);
