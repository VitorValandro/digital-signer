...
export function SignatureCard({createdAt, signatureUrl}: SignatureCardProps) {
  const [imageUrl, setImageUrl] = useState("");
  const [showModal, setShowModal] = useState(false);

  useEffect(() => {
    const getFile = async () => {
      const {file, fileName} = await storageProvider.download(signatureUrl);
  ...
  return (
    <>
      <div className="group p-0 max-w-sm h-72 bg-gradient-to-t from-slate-200 via white to-slate-100 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        <div className="h-1/3 bg-slate-100 rounded-lg rounded-t-[40px]">
          <div className="flex flex-col sm:flex-row justify-between items-center text-center p-5">
            <p className="transition-all ease-in-out duration-300 font-light text-slate-500 dark:text-gray-400">
              • Adicionada em {new Date(createdAt).toLocaleDateString("pt-br")}
            </p>
            <button
              onClick={() => {
                console.log("a");
      ...
      {showModal && (
        <ConfirmationModal
          message="Você tem certeza que deseja excluir essa assinatura?"
